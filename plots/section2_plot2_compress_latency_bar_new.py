...
bar1 = ax.broken_barh([[0,latency[0]]], [1-bar_width/2, bar_width], label=conditions[0], facecolors='tab:blue', edgecolor=bar_edge_color)
bar2 = ax.broken_barh([[0,latency[1]]], [0-bar_width/2, bar_width], label=conditions[0], facecolors='tab:blue', edgecolor=bar_edge_color)
bar3 = ax.broken_barh([[0,latency[2]]], [-1-bar_width/2, bar_width], label=conditions[0], facecolors='tab:blue', edgecolor=bar_edge_color)
...
#                 [cidr_hash+cidr_lookup+cidr_compress+cidr_transfer, cidr_overhead]], [0-bar_width/2, bar_width], label=conditions[1], facecolors='tab:orange', edgecolor=bar_edge_color)
bar2_h = ax.broken_barh([[0,cidr_hash],                                                      ], [0-bar_width/2, bar_width], label=conditions[1], facecolors='tab:orange', edgecolor=bar_edge_color)
bar2_c = ax.broken_barh([[cidr_hash, cidr_compress],                                         ], [0-bar_width/2, bar_width], label=conditions[1], facecolors='tab:green', edgecolor=bar_edge_color) 
bar2_t = ax.broken_barh([[cidr_hash+cidr_compress, cidr_transfer],                           ], [0-bar_width/2, bar_width], label=conditions[1], facecolors='tab:red', edgecolor=bar_edge_color)
# bar2_o = ax.broken_barh([[cidr_hash+cidr_compress+cidr_transfer, cidr_lookup + cidr_overhead]], [0-bar_width/2, bar_width], label=conditions[1], facecolors='none', edgecolor=bar_edge_color)
bar3_h = ax.broken_barh([[0,stream_dedup_hash],                                                              ], [-1-bar_width/2, bar_width], label=conditions[2], facecolors='tab:orange', edgecolor=bar_edge_color)
bar3_c = ax.broken_barh([[stream_dedup_hash, stream_dedup_compress],                                         ], [-1-bar_width/2, bar_width], label=conditions[2], facecolors='tab:green', edgecolor=bar_edge_color)
bar3_t = ax.broken_barh([[stream_dedup_hash+stream_dedup_compress, stream_dedup_transfer],                   ], [-1-bar_width/2, bar_width], label=conditions[2], facecolors='tab:red', edgecolor=bar_edge_color)
# bar3_o = ax.broken_barh([[stream_dedup_hash+stream_dedup_compress+stream_dedup_transfer, stream_dedup_lookup]], [-1-bar_width/2, bar_width], label=conditions[2], facecolors='none', edgecolor=bar_edge_color)
...
ax.annotate(f"Hash Table Lookup & Control Overhead", xy=(cidr_hash+cidr_compress+cidr_transfer+(cidr_lookup + cidr_overhead)/5, 0), xytext=(1000, -0.5), fontsize = 8,
                arrowprops=arrow_config, ha='center', va='center')
ax.annotate(f"Hash Table Lookup & Control Overhead", xy=(stream_dedup_hash+stream_dedup_compress+stream_dedup_transfer+ (stream_dedup_lookup)/5, -0.9), xytext=(1000, -0.5), fontsize = 8,
                arrowprops=arrow_config, ha='center', va='center')
...
lines = [bar1, bar2_h, bar2_c, bar2_t]
labels = ['Overall', 'Fingerprint', '(De-)Compression', 'Data Movement']
# legend=ax1.legend(lines, labels, loc='upper left')
legend=plt.legend(lines, labels,loc='upper center', bbox_to_anchor=(0.5, 1.2), ncol=4, handlelength=0.7, handletextpad=0.5, markerfirst=True)
# legend = plt.legend()
...
plt.subplots_adjust(left=0.12, right=0.98, top=0.87, bottom=0.15)
# Alternatively
# plt.tight_layout()
...
save_plot('./section2', 'plot2_compress_latency_bar_new')
