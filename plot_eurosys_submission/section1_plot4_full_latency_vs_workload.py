...
latency_1 = [(a/1000) for a in baseline_full]
latency_2 = [(a/1000) for a in stream_dedup_full]
improvement = [a/b for a, b in zip(latency_1, latency_2)]
print(f'{improvement=}') 
# SHA2 improvement=[17.299029451137883, 724.4149842363953, 1.1708199356913183, 1.2048491351779465, 0.9847143098877544]
# SHA3 improvement=[58.5115954012127, 59.9502614212701, 1.1708199356913183, 1.2048491351779465, 0.9847143098877544]
# Number of cases
n_cases = len(cases)
...
plt.plot([-0.5, 1.5], [25.5, 25.5], color='r', linestyle='dashed', linewidth=0.5, zorder=5)
plt.text(0.5, 25.5*1.02, f"25.5 us", fontsize = 7, rotation=0, rotation_mode='anchor', weight = 'bold', ha = 'center', va = 'bottom')

# plt.axhline(y = 10, color = 'r', linestyle = 'dashed', linewidth = 0.5, zorder = 5)
plt.plot([1.5, 4.5], [12, 12], color='r', linestyle='dashed', linewidth=0.5, zorder=5)
plt.text(3, 12*1.02, f"12 us", fontsize = 7, rotation=0, rotation_mode='anchor', weight = 'bold', ha = 'center', va = 'bottom')
...
save_plot('./section1', 'plot4_latency_workload_bar_full')
