...
# baseline = [2.08475e+07, 2.078865e+07, 1.5509775e+10]
# stream_dedup = [5284910.0, 5285320.0, 5358210.0]

#SHA3
# insertion: 16 thread, slightly faster than 32 thread
# ./main -n 16384 -d 0 -f 0.99 -h 64
# ./main -n 16384 -g 0 -f 0.5 -o 2
baseline = [2.35026e+07, 2.212275e+07, 1.5430925e+10]
stream_dedup = [5284910.0, 5285320.0, 5358210.0]

throughput_1 = [16384*4.096/(a/1000) for a in baseline]
throughput_2 = [16384*4.096/(a/1000) for a in stream_dedup]
...
bar1 = ax.bar(index - (bar_dist + bar_width)/2, throughput_1, bar_width, label=conditions[0], log=True)
bar2 = ax.bar(index + (bar_dist + bar_width)/2, throughput_2, bar_width, label=conditions[1], log=True)
...
save_plot('./section1', 'plot5_no_dup_write_throughput_vs_fullness')
