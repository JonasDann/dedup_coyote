...
std::queue<std::pair<size_t, std::vector<unsigned char>>> hash_queue;
int active_hash_workers = 0;

std::mutex compress_queue_mutex;
std::condition_variable compress_queue_cv;
std::queue<size_t> compress_queue;
bool hash_processing_done = false;

///////////////////////////////////////////
// Stage 1: Multi-Threaded Hash Computation
///////////////////////////////////////////
// void hash_page(char* page, size_t page_idx) {
...
//     hash_queue.push({page_idx, hash_vec});
//   }
//   queue_cv.notify_one();
// }
// SHA3-256
...
void worker(char* page_buffer, size_t total_page_count, size_t num_threads, size_t worker_id) {
  ...
  for (size_t i = start_page; i < end_page; ++i) {
    hash_page(page_buffer + i * pg_size, i);
  }
  {
    std::lock_guard<std::mutex> lock(queue_mutex);
    active_hash_workers--;
...
/////////////////////////////////////////////
// Stage 2: Single-Threaded Hash Table Lookup
/////////////////////////////////////////////
void process_hashes(struct dedup_config* dc, int32_t op, int32_t* status, uint64_t *counter, std::vector<bool> need_compression) {
  while (true) {
    std::unique_lock<std::mutex> lock(queue_mutex);
    queue_cv.wait(lock, [] { return !hash_queue.empty() || (active_hash_workers == 0); });
    while (!hash_queue.empty()) {
      auto [page_idx, hash] = hash_queue.front();
      hash_queue.pop();
      lock.unlock();
      ...
      if (need_compression[page_idx]) {
        std::lock_guard<std::mutex> compress_lock(compress_queue_mutex);
        compress_queue.push(page_idx);
        compress_queue_cv.notify_one();
      }
      lock.lock();
    }

    // if ((active_hash_workers == 0) && hash_queue.empty()) break;
    if ((active_hash_workers == 0) && hash_queue.empty()) {
      std::lock_guard<std::mutex> compress_lock(compress_queue_mutex);
      hash_processing_done = true;
      compress_queue_cv.notify_all();
      break;
    }
  }
}
...
/////////////////////////////////////////////
// Stage 3: Multi-Threaded Page Compression
/////////////////////////////////////////////
// gzip using zlib, see https://github.com/mapbox/gzip-hpp/blob/master/include/gzip/compress.hpp
std::vector<char> compress_page(const char* buffer, size_t page_idx) {
  // Allocate memory for the output (compressed) buffer
  // The worst case scenario is that the compressed size will be slightly larger than the input size
  uLongf compressed_size = compressBound(pg_size) + 18; // Adding space for gzip header and footer
  std::vector<char> compressed_page(compressed_size);
  ...
  if (ret != Z_OK) {
    std::cerr << "deflateInit2 failed with error code: " << ret << std::endl;
    return std::vector<char>();
  }

  // Set input data
  strm.next_in = reinterpret_cast<Bytef*>(const_cast<char*>(buffer) + page_idx * pg_size);
  strm.avail_in = pg_size;

  // Set output buffer
  strm.next_out = reinterpret_cast<Bytef*>(compressed_page.data());
  ...
  compressed_page.resize(compressed_size);
  return compressed_page;
}

void run_compression(char* page_buffer, uint64_t *compression_counter) {
  while (true) {
    std::unique_lock<std::mutex> lock(compress_queue_mutex);
    compress_queue_cv.wait(lock, [] { return !compress_queue.empty() || hash_processing_done; });

    while (!compress_queue.empty()) {
      size_t page_idx = compress_queue.front();
      compress_queue.pop();
      lock.unlock();

      compress_page(page_buffer, page_idx);

      lock.lock();
      *compression_counter = *compression_counter + 1;
    }

    if (hash_processing_done && compress_queue.empty()) break;
  }
}

void hash_table_process(char* page_buffer, size_t total_page_count, std::vector<bool> need_compression,
                        size_t num_hash_threads, size_t num_compress_threads, 
                        struct dedup_config* dc, int32_t op, int32_t* status, 
                        uint64_t *lookup_counter, uint64_t *compression_counter, bool do_compression) {
  std::vector<std::thread> hash_threads;
  active_hash_workers = num_hash_threads;
  hash_processing_done = false;

  for (size_t i = 0; i < num_hash_threads; ++i) {
    hash_threads.emplace_back(worker, page_buffer, total_page_count, num_hash_threads, i);
  }

  std::thread hash_table_thread(process_hashes, dc, op, status, lookup_counter, need_compression);

  
  std::vector<std::thread> compress_threads;
  if (do_compression) {
    for (size_t i = 0; i < num_compress_threads; ++i) {
      compress_threads.emplace_back(run_compression, page_buffer, compression_counter);
    }
  }

  for (auto& thread : hash_threads) {
    thread.join();
  }
  hash_table_thread.join();
  if (do_compression) {
    for (auto& thread : compress_threads) {
      thread.join();
    }
  }
}
...
{ 
  ...
  ("numCompressThreads,c", boost::program_options::value<uint64_t>()->default_value(8), "Number of thread for compression")
  ("enableCompress,e", boost::program_options::value<bool>()->default_value(true), "Enable compression operation");
  ...
  size_t num_hash_threads = commandLineArgs["numHashThreads"].as<size_t>();
  size_t num_compress_threads = commandLineArgs["numCompressThreads"].as<size_t>();
  bool enable_compress = commandLineArgs["enableCompress"].as<bool>();
  ...
  close(urand);
  // memset(all_unique_page_buffer, 0, total_page_unique_count * pg_size);
  ...
  /* step 1  insert*/
  int32_t op_status;
  uint64_t lookup_counter = 0;
  uint64_t compress_counter = 0;
  std::vector<bool> init_need_compression(total_old_page_unique_count, false);
  ...
  hash_table_process(all_unique_page_buffer, total_old_page_unique_count, init_need_compression,
                      num_hash_threads, num_compress_threads, 
                      dc, 1, &op_status, 
                      &lookup_counter, &compress_counter, false);
  ...
  double time = std::chrono::duration_cast<std::chrono::nanoseconds>(end_time - begin_time).count();
  std::cout << "time used: " << time << "ns" << std::endl;
  std::cout << "page lookup: " << lookup_counter << " / " << total_old_page_unique_count << std::endl;
  std::cout << "page compress: " << compress_counter << " / " << 0 << std::endl;

  /* step 2 benchmark*/
  uint64_t benchmark_insert_pg_num = n_page;
  std::vector<bool> benchmark_need_compression(benchmark_insert_pg_num, false);
  ...
  for (uint64_t i = 0; i < benchmark_insert_pg_num; ++i) {
    ...
    if (page_idx >= total_old_page_unique_count){
      // new page and need compression
      benchmark_need_compression[i] = true;
    }
  }
  lookup_counter = 0;
  compress_counter = 0;
  ...
  hash_table_process(benchmark_page_buffer, benchmark_insert_pg_num, benchmark_need_compression,
                      num_hash_threads, num_compress_threads, 
                      dc, 1, &op_status, 
                      &lookup_counter, &compress_counter, enable_compress);
  // hash_table_process(benchmark_page_buffer, benchmark_insert_pg_num, benchmark_need_compression,
  //                     num_hash_threads, num_compress_threads, 
  //                     dc, 1, &op_status, 
  //                     nullptr, nullptr, enable_compress);
  // for (int i = 0; i < benchmark_insert_pg_num; i ++){
  ...
  std::cout << "page lookup: " << lookup_counter << " / " << benchmark_insert_pg_num << std::endl;
  std::cout << "page compress enabled: " << enable_compress  << std::endl;
  std::cout << "page compress: " << compress_counter << " / " << batch_new_page_unique_count << std::endl;
