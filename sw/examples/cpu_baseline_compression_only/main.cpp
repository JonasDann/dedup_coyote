...
void worker(char* page_buffer, size_t total_page_count, size_t num_threads, size_t worker_id, std::vector<std::vector<char>>& compressed_data, std::mutex& data_mutex) {
  ...
  for (size_t i = start_page; i < end_page; ++i) {
    auto compressed_page = compress_page(page_buffer, i);
    std::lock_guard<std::mutex> lock(data_mutex);
    compressed_data[i] = std::move(compressed_page);
  }
}

// Function to run the compression with multiple threads
void run_compression(char* page_buffer, size_t total_page_count, size_t num_compress_threads) {
  std::vector<std::thread> compress_threads;
  std::vector<std::vector<char>> compressed_data(total_page_count);
  std::mutex data_mutex;

  for (size_t i = 0; i < num_compress_threads; ++i) {
    compress_threads.emplace_back(worker, page_buffer, total_page_count, num_compress_threads, i, std::ref(compressed_data), std::ref(data_mutex));
  }
  ...
  // // Handle the compressed_data as needed
  // for (size_t i = 0; i < total_page_count; ++i) {
  //   std::cout << "Page " << i << " compressed size: " << compressed_data[i].size() << " bytes" << std::endl;
  // }
}
...
{ 
  ...
  char* all_unique_page_buffer = (char*) malloc(n_page * pg_size); // 4kiB Pages
  ...
  int res = read(urand, all_unique_page_buffer, n_page * pg_size);
  close(urand);
  // memset(all_unique_page_buffer, 0, n_page * pg_size);

  std::cout << "benchmarking...... "<< std::endl;
  auto bench_begin_time = std::chrono::high_resolution_clock::now();
  run_compression(all_unique_page_buffer, n_page, num_compress_threads);
