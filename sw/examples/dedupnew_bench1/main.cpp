...
const int routing_channel_count = 8;
...
{   
  ...
  bool is_active = (bool) commandLineArgs["isActive"].as<uint32_t>();
  ...
  string output_dir = project_dir + "/sw/examples/dedupnew_bench1/page_resp";
  // string 
  ...
  if (mkdir(output_dir.c_str(), 0775) == -1) {
    if (errno == EEXIST) {
      std::cout << "Directory already exists: " << output_dir << std::endl;
    } else {
  ...
  // setup routing table
  uint32_t sim_node_idx = 7;
  uint32_t active_channel_count = 1;
  cproc.setCSR(active_channel_count, static_cast<uint32_t>(CTLR::ACTIVE_CHANNEL_COUNT));
  for (int index = 0 ; index < routing_channel_count; index ++){
    // nodeIdx
    if (index == 0) {
      cproc.setCSR(sim_node_idx, static_cast<uint32_t>(static_cast<uint32_t>(CTLR::ROUTING_TABLE_CONTENT_BASE) + 3 * index));
    } else if (index == 1){
      cproc.setCSR(sim_node_idx - 1, static_cast<uint32_t>(static_cast<uint32_t>(CTLR::ROUTING_TABLE_CONTENT_BASE) + 3 * index));
    } else {
      cproc.setCSR(sim_node_idx + (index - 1), static_cast<uint32_t>(static_cast<uint32_t>(CTLR::ROUTING_TABLE_CONTENT_BASE) + 3 * index));
    }
    // start
    cproc.setCSR(0 , static_cast<uint32_t>(static_cast<uint32_t>(CTLR::ROUTING_TABLE_CONTENT_BASE) + 1 + 3 * index));
    // len
    if (index == 0){
      cproc.setCSR(1 << 16 , static_cast<uint32_t>(static_cast<uint32_t>(CTLR::ROUTING_TABLE_CONTENT_BASE) + 2 + 3 * index));
    } else {
      cproc.setCSR(0 , static_cast<uint32_t>(static_cast<uint32_t>(CTLR::ROUTING_TABLE_CONTENT_BASE) + 2 + 3 * index));
    }
  }
